...
describe('LinkedIn Comment Generator Tests', () => {
    ...
    beforeEach(() => {
        ...
        post = TestUtils.createMockPost('linkedin', 'Test LinkedIn post content');
        commentField = TestUtils.createMockCommentField('linkedin');
    ...
    describe('Comment Selection & Insertion', () => {
        test('should insert selected comment correctly', async () => {
            const testComment = 'Test comment for LinkedIn';
    ...
    describe('Platform Integration', () => {
        test('should detect LinkedIn environment correctly', () => {
            expect(window.location.href.includes('linkedin.com')).toBe(true);
        });

        test('should handle platform-specific DOM elements', () => {
            expect(post.classList.contains('feed-shared-update-v2')).toBe(true);
            expect(commentField.classList.contains('comments-comment-box')).toBe(true);
        });
    });
});
